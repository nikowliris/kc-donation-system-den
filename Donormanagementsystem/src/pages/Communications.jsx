...
import { Mail, Send, Plus, Settings, Clock, CheckCircle, Trash2, Inbox, X, Save, Zap } from 'lucide-react';
...
const defaultWorkflows = [
  { id: 1, name: 'New Donor Welcome Series', trigger: 'New Donor Added', status: 'Active', steps: 3, description: 'Automatically sends a welcome email series to new donors over 3 days.', delay: '24', templateId: 3 },
  { id: 2, name: 'Donation Receipt', trigger: 'Donation Received', status: 'Active', steps: 1, description: 'Sends an instant thank-you receipt when a donation is recorded.', delay: '0', templateId: 1 },
  { id: 3, name: 'Lapsed Donor Re-engagement', trigger: 'No Donation > 6 months', status: 'Paused', steps: 2, description: 'Re-engages donors who have not donated in over 6 months.', delay: '48', templateId: 2 },
];
              ...
              <CardHeader>
                <CardTitle className="flex justify-between items-start">
                  ...
                  <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    <Button variant="ghost" size="sm" onClick={(e) => { e.stopPropagation(); setTemplateModal(template); }}>
                      Edit
                    </Button>
                    <button
              ...
              <CardContent className="p-4 flex items-center justify-between">
                <div className="flex items-center space-x-4">
                  <div className={`p-2 rounded-lg ${workflow.status === 'Active' ? 'bg-green-100' : 'bg-gray-100'}`}>
                    <Zap className={`h-6 w-6 ${workflow.status === 'Active' ? 'text-green-600' : 'text-gray-400'}`} />
                  </div>
                  <div>
                    <h3 className="text-base font-medium text-gray-900">{workflow.name}</h3>
                    <p className="text-sm text-gray-500">
                      Trigger: {workflow.trigger} · {workflow.steps} step{workflow.steps !== 1 ? 's' : ''}
                    ...
                    {workflow.templateId && (
                      <p className="text-xs text-blue-600 mt-0.5">
                        Template: {templates.find((t) => t.id === workflow.templateId)?.name || 'Unknown'}
                      </p>
                    )}
                  </div>
                </div>
                <div className="flex items-center space-x-3">
                  ...
                  <Button variant="outline" size="sm" onClick={() => setWorkflowModal(workflow)}>
                    Configure
                  </Button>
                </div>
              </CardContent>
                      ...
                      <div key={item.id} className={`flex ${isAdmin ? 'justify-end' : 'justify-start'}`}>
                        <div className="max-w-[75%]">
                          ...
                          <div className={`rounded-2xl px-4 py-3 text-sm whitespace-pre-wrap leading-relaxed ${
                            isAdmin ? 'bg-blue-600 text-white rounded-tr-sm' : 'bg-gray-100 text-gray-800 rounded-tl-sm'
                          }`}>
                            {item.text}
                          </div>
                        </div>
